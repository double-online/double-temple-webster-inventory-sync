...
async function exportProductsToCSV() {
  ...
  for (const product of allProducts) {
    ...
    for (const variant of product.variants) {
      ...
      let allLocationsEmpty = true;
      
      // First pass: collect inventory data and check if all locations are empty
      locations.forEach(location => {
        const level = inventoryLevels.find(l => l.location_id === location.id);
        const locationColumn = locationMap[location.id];
        const available = level ? level.available : 0;
        inventoryByLocation[locationColumn] = available;
        
        // Check if any location has stock/availability > 0
        if (available > 0) {
      ...
      if (!allLocationsEmpty) {
        ...
        if (mainLocation) {
          const level = inventoryLevels.find(l => l.location_id === mainLocation.id);
          if (!level || level.available === 0) {
